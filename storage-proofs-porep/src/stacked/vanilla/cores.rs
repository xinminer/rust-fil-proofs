...
fn custom_core_units(cfg: String) -> Option<Vec<Mutex<Vec<CoreIndex>>>> {
    let groups = cfg.split("|").collect::<Vec<_>>();
    let cores = groups.iter().map(|group|{ group.split(",").map(|x|x.parse::<usize>().unwrap()).collect::<Vec<_>>() }).collect::<Vec<_>>();
    debug!("Core units: {:?}", cores);

    Some(
        cores
            .iter()
            .map(|group| {
                let group_index = group.iter().map(|core| CoreIndex(*core)).collect::<Vec<_>>();
                Mutex::new(group_index)
            })
            .collect::<Vec<_>>()
    )
}
